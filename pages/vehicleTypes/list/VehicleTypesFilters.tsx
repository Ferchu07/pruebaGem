...
const VehicleTypesFilters: React.FC<VehicleTypesFiltersProps> = ({ filters, updateFilters, resetFilters }) => {

    // STATES

    const [refresh, setRefresh] = useState(0);
    const { fetchVehicleSubTypes } = useVehicleSubTypes({ status: true });
        ...
        <>
            ...
            { userCan('get_vehicle_subtypes_for_selects', 'vehicle_subtypes') && (
                <CustomSelect
                    ...
                    onChange={(e: any) => {
                        updateFilters({ vehicleSubtype: e.value });
                    }}
                    value={vehicleSubtypeOptions.find((vehicleSubtype) => vehicleSubtype.value === filters.filter_filters?.vehicleSubtype)}
                />
            )}
...
export default VehicleTypesFilters;
